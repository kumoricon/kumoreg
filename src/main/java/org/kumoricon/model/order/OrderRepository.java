...
public interface OrderRepository extends JpaRepository<Order, Integer> {
    ...
    @Query(value = "SELECT users.id, users.username, users.firstName, users.lastName, paidSession, paymentType, COUNT(orders.id) as cnt, SUM(totalAmount) as total FROM orders, users WHERE paymentTakenByUser_id IS NOT NULL AND paymentTakenByUser_id = users.id GROUP BY paymentTakenByUser_id, paidSession, paymentType;", nativeQuery = true)
